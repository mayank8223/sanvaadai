...
- **Improvement Notes:**
  ...
  - (2026-03-08) **T29 completed:** Zod schemas for AI outputs in `apps/web/lib/ai/schemas.ts`: `aiFormDefinitionDraftSchema` (title, description, fields with all types), `answerDraftSchema` (answers, missingRequiredFields, followUpQuestions). Exported types: `AiFormDefinitionDraft`, `AiFormFieldDraft`, `AnswerDraft`, `FollowUpQuestion`. Unit tests in `lib/ai/schemas.test.ts`. See `docs/T29-ai-schemas-checklist.md`.
  - (2026-03-08) **T30 completed:** AI endpoint: `POST /api/ai/forms/generate` in `apps/web/app/api/ai/forms/generate/route.ts`. Accepts `{ description }`, returns `{ form }` (FormBuilderSubmitPayload). Uses `generateText` + `Output.object` with `aiFormDefinitionDraftSchema`. Added `lib/ai/form-generate.ts` with `generateFormFromDescription()` and `aiDraftToBuilderPayload()`. Admin-only. See `docs/T30-ai-form-generate-checklist.md`.
  - (2026-03-08) **T31 completed:** AI assistant panel for form creation in `apps/web/components/forms/form-builder-ai-panel.tsx`. Slide-over panel with text area, mic button (record → transcribe → populate), Generate form button, and Apply to builder. Hooks: `useAiFormAssistant`, `useAudioRecorder`. Extended `useFormBuilder` with `applyFormDraft()`. See `docs/T31-ai-assistant-panel-checklist.md`.
  - (2026-03-08) **T32 completed:** AI endpoint: `POST /api/ai/answers/draft` in `apps/web/app/api/ai/answers/draft/route.ts`. Accepts `{ transcription, formDefinition: { fields } }`, returns `{ answerDraft }`. Added `lib/ai/answer-draft.ts` with `generateAnswerDraftFromTranscription()`. Uses `answerDraftSchema`. Auth required (collectors can use). See `docs/T32-ai-answer-draft-checklist.md`.
  - (2026-03-08) **T33 completed:** Mobile voice-powered form filling in `apps/mobile`: `useVoiceFormFill` hook (expo-av recording, transcribe + answer draft API), `applyDraftAnswers` in `useDynamicFormDraft`, "Fill via voice" button and follow-up questions in `DynamicFormScreen`. API clients: `lib/api/transcribe.ts`, `lib/api/answer-draft.ts`. See `docs/T33-mobile-voice-form-fill-checklist.md`.
  - (2026-03-08) **T34 completed:** Web collector AI flow at `/collector/forms/[id]`. `CollectorFormFillClient` with voice assistant (record/type → transcribe → answer draft → apply). `useAiAnswerAssistant`, `lib/forms/collector-fill.ts`. Collector home forms link to fill page. See `docs/T34-web-collector-ai-flow-checklist.md`.
